...
// Constants are defined inline to avoid pulling in <GL/glext.h>; function
// pointers are resolved at runtime via SDL_GL_GetProcAddress and degrade
// gracefully when unavailable (e.g. software renderer).
enum {
  ...
  SWELL_GL_TEXTURE_RECTANGLE         = 0x84F5,
...
static struct {
  ...
  void    (*BindTexture  )(unsigned target, unsigned tex);
} s_pbo_gl;
...
#define SWELL_LOAD(fn, name) s_pbo_gl.fn = (decltype(s_pbo_gl.fn))SDL_GL_GetProcAddress(name)
  ...
  SWELL_LOAD(BindTexture,    "glBindTexture");
#undef SWELL_LOAD
  s_pbo_gl_ok = s_pbo_gl.GenBuffers && s_pbo_gl.DeleteBuffers &&
                s_pbo_gl.BindBuffer && s_pbo_gl.BufferData &&
                s_pbo_gl.MapBufferRange && s_pbo_gl.UnmapBuffer &&
                s_pbo_gl.TexSubImage2D && s_pbo_gl.PixelStorei &&
                s_pbo_gl.BindTexture;
  return s_pbo_gl_ok;
}
...
#ifdef SWELL_SKIA_GDI
static bool swell_sdl_want_skia_gpu()
{
  const char *v = getenv("SWELL_SKIA_GPU");
  return !v || !*v || atoi(v) != 0;
}

static bool swell_sdl_is_skia_gpu_window(swell_sdl_window_state *st)
{
  return st && st->glctx && st->skia_gpu_context;
}
#endif
...
#ifdef SWELL_SKIA_GDI
// Upload one or more dirty rects via PBO under a single texture bind/flush.
//
// SDL_RenderFlush() is required before raw GL calls (SDL docs). Doing it once
// per batch rather than once per rect avoids redundant driver round-trips.
// SDL_GL_BindTexture returns normalised texcoords (tw==th==1 → GL_TEXTURE_2D,
// pixel dims → GL_TEXTURE_RECTANGLE). We unbind with a direct glBindTexture(0)
// instead of SDL_GL_UnbindTexture, which calls glDisable(target) and corrupts
// SDL's fixed-function state cache.
static bool swell_sdl_upload_texture_rects_pbo(swell_sdl_window_state *st,
                                               LICE_IBitmap *bm,
                                               const RECT *rects, int rect_count)
{
  if (rect_count <= 0 || !rects) return false;

  SDL_RenderFlush(st->renderer);

  float tw = 0.0f, th = 0.0f;
  if (SDL_GL_BindTexture(st->texture, &tw, &th) != 0) return false;
  const unsigned tex_target = (tw == 1.0f && th == 1.0f)
                              ? SWELL_GL_TEXTURE_2D : SWELL_GL_TEXTURE_RECTANGLE;

  const int src_pitch = bm->getRowSpan() * 4;
  const char *bits = (const char *)bm->getBits();
  bool ok = true;
  ...
  for (int i = 0; i < rect_count; i++)
  {
    ...
    {
      // Orphan + grow: gives driver fresh memory, avoids GPU stall.
    ...
    if (!dst) { ok = false; s_pbo_gl.BindBuffer(SWELL_GL_PIXEL_UNPACK_BUFFER, 0); continue; }

    const char *s = bits + r.top * src_pitch + r.left * 4;
    char *d = (char *)dst;
...
#ifdef SWELL_SKIA_GDI
  if (st->m_pbo && swell_sdl_upload_texture_rects_pbo(st, bm, r, 1)) return true;
#endif
...
#ifdef SWELL_SKIA_GDI
    if (st->m_pbo && bm->getBits())
      return swell_sdl_upload_texture_rects_pbo(st, bm, rects, rect_count);
#endif
...
#ifdef SWELL_SKIA_GDI
static bool swell_sdl_present_skia_gpu(swell_sdl_window_state *st)
{
  if (!swell_sdl_is_skia_gpu_window(st) || !st->hwnd || !st->hwnd->m_backingstore) return false;
  if (!SWELL_IsSkiaGPUBitmap(st->hwnd->m_backingstore)) return false;
  if (SDL_GL_MakeCurrent(st->window, st->glctx) != 0) return false;

  int dw = 0, dh = 0;
  SDL_GL_GetDrawableSize(st->window, &dw, &dh);
  if (dw <= 0 || dh <= 0) SDL_GetWindowSize(st->window, &dw, &dh);
  if (dw <= 0 || dh <= 0) return false;

  if (!SWELL_PresentSkiaGPUBitmapToGLFramebuffer(st->skia_gpu_context, st->hwnd->m_backingstore, dw, dh))
    return false;
  SDL_GL_SwapWindow(st->window);

  // Throttle deferred cleanup to at most once every ~5 seconds so the cache
  // scan doesn't burn time at high frame rates.
  static Uint32 s_last_cleanup_ticks = 0;
  const Uint32 now = SDL_GetTicks();
  if (now - s_last_cleanup_ticks >= 5000)
  {
    SWELL_SkiaGPUContextPeriodicCleanup(st->skia_gpu_context);
    s_last_cleanup_ticks = now;
  }
  return true;
}
#endif
...
static void swell_sdl_paint(HWND hwnd, const RECT *dirty)
{
  ...
  if (!st || !hwnd || !hwnd->m_oswindow) return;
#ifdef SWELL_SKIA_GDI
  const bool use_skia_gpu = swell_sdl_is_skia_gpu_window(st);
#else
  const bool use_skia_gpu = false;
#endif
  if (!use_skia_gpu && !st->renderer) return;
...
#ifdef SWELL_SKIA_GDI
  if (use_skia_gpu && SDL_GL_MakeCurrent(st->window, st->glctx) != 0)
  {
    s_sdl_paint_depth--;
    return;
  }
  ...
  bool force_full_paint = false;
  if (!hwnd->m_backingstore)
  {
#ifdef SWELL_SKIA_GDI
    hwnd->m_backingstore = use_skia_gpu ?
      SWELL_CreateSkiaGPUBitmap(st->skia_gpu_context, cr.right, cr.bottom) :
      SWELL_CreateSkiaRasterBitmap(cr.right, cr.bottom);
    if (!hwnd->m_backingstore && !use_skia_gpu) hwnd->m_backingstore = new LICE_SysBitmap;
#else
    ...
    force_full_paint = true;
  }
#ifdef SWELL_SKIA_GDI
  else if (use_skia_gpu && !SWELL_IsSkiaGPUBitmap(hwnd->m_backingstore))
  {
    delete hwnd->m_backingstore;
    hwnd->m_backingstore = SWELL_CreateSkiaGPUBitmap(st->skia_gpu_context, cr.right, cr.bottom);
    if (!hwnd->m_backingstore)
    {
      s_sdl_paint_depth--;
      return;
...
#endif
  const bool resized_backingstore = hwnd->m_backingstore->resize(cr.right, cr.bottom);
  bool forceref = force_full_paint || resized_backingstore;
  ...
  // Save dirty sub-rects before clearing them: after painting we upload only
  // those sub-rects rather than the full bounding box, cutting GPU traffic.
  RECT saved_rects[8];
...
#ifdef SWELL_LICE_GDI
  swell_sdl_window_state *st = swell_sdl_state_from_hwnd(hwnd);
  if (!st || !hwnd || !hwnd->m_backingstore || !hwnd->m_oswindow) return false;
#ifdef SWELL_SKIA_GDI
  if (swell_sdl_is_skia_gpu_window(st))
    return swell_sdl_present_skia_gpu(st);
#endif
...
static bool swell_sdl_paint_initial(HWND hwnd)
{
  ...
  if (!st) return false;
#ifdef SWELL_SKIA_GDI
  if (!swell_sdl_is_skia_gpu_window(st) && !st->renderer) return false;
#else
  if (!st->renderer) return false;
#endif
  swell_sdl_paint(hwnd, NULL);
  return st->texture != NULL || hwnd->m_backingstore != NULL;
#else
  return false;
#endif
}
...
#ifdef SWELL_LICE_GDI
#ifdef SWELL_SKIA_GDI
    if (swell_sdl_is_skia_gpu_window(st) && hwnd && hwnd->m_backingstore)
    {
...
#ifdef SWELL_SKIA_GDI
    if (st->skia_gpu_context)
    {
      if (st->glctx) SDL_GL_MakeCurrent(st->window, st->glctx);
      SWELL_DestroySkiaGPUContext(st->skia_gpu_context);
    }
    if (st->glctx) SDL_GL_DeleteContext(st->glctx);
#endif
...
void swell_oswindow_manage(HWND hwnd, bool wantfocus)
{
  ...
  if (isVis != wantVis)
  {
    ...
    else if (swell_sdl_initwindowsys())
    {
      ...
      const Uint32 base_flags = swell_sdl_window_flags(hwnd);
      Uint32 create_flags = base_flags;
#ifdef SWELL_SKIA_GDI
      const bool try_skia_gpu = swell_sdl_want_skia_gpu();
      bool created_with_gl = false;
      if (try_skia_gpu)
      {
        SDL_GL_SetAttribute(SDL_GL_DOUBLEBUFFER, 1);
        SDL_GL_SetAttribute(SDL_GL_DEPTH_SIZE, 0);
        SDL_GL_SetAttribute(SDL_GL_STENCIL_SIZE, 8);
        create_flags |= SDL_WINDOW_OPENGL;
        created_with_gl = true;
      }
#endif
      SDL_Window *window = SDL_CreateWindow(hwnd->m_title.Get(), create_x, create_y, w, h, create_flags);
#ifdef SWELL_SKIA_GDI
      SDL_GLContext glctx = NULL;
      void *skia_gpu_context = NULL;
      if (!window && try_skia_gpu)
      {
        created_with_gl = false;
        window = SDL_CreateWindow(hwnd->m_title.Get(), create_x, create_y, w, h, base_flags);
      }
      if (window && try_skia_gpu && created_with_gl)
      {
        glctx = SDL_GL_CreateContext(window);
        if (glctx && SDL_GL_MakeCurrent(window, glctx) == 0)
          skia_gpu_context = SWELL_CreateSkiaGLGPUContext();
        if (!skia_gpu_context)
        {
...
#ifdef SWELL_SKIA_GDI
        st->glctx = glctx;
        st->skia_gpu_context = skia_gpu_context;
        if (!st->skia_gpu_context)
#endif
        {
