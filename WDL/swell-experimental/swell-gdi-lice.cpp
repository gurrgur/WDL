...
{
#ifdef SWELL_SKIA_GDI
  // If the source DC is GPU-backed, create a GPU bitmap so that all GDI
  // operations on this context stay on the GPU and the eventual BitBlt
  // into the window DC becomes a GPU-to-GPU copy with no CPU roundtrip.
  LICE_IBitmap *bm = NULL;
  if (HDC_VALID((HDC__ *)hdc))
  {
    HDC__ *src = (HDC__ *)hdc;
    void *gpu_ctx = src->surface ? SWELL_GetGPUContextFromBitmap(src->surface) : NULL;
    if (gpu_ctx) bm = SWELL_CreateSkiaGPUBitmap(gpu_ctx, w, h);
    // GPU bitmap is pre-cleared by resize(); skip LICE_Clear below for that path.
  }
  const bool is_gpu = bm != NULL;
  if (!bm) bm = SWELL_CreateSkiaRasterBitmap(w, h);
#else
  LICE_MemBitmap * bm = new LICE_MemBitmap(w,h);
  const bool is_gpu = false;
#endif
  if (WDL_NOT_NORMALLY(!bm)) return 0;
  if (!is_gpu) LICE_Clear(bm,LICE_RGBA(0,0,0,0));
...
#if defined(SWELL_SKIA_GDI)
static int swell_skia_simple_text_run_len(const char *buf, int buflen, int align)
{
  (void)buf;
  (void)buflen;
  (void)align;

  // Keep DrawText on the per-codepoint path. Passing arbitrary SWELL text as
  // one UTF-8 run into SkFont::measureText()/drawSimpleText() can crash inside
  // SkTypeface_FreeType::onCharsToGlyphs() for some long scrollable edit text.
  return -1;
}

static int swell_skia_round_text_extent(float v, int fallback)
{
...
int DrawText(HDC ctx, const char *buf, int buflen, RECT *r, int align)
{
  ...
  int charw = 8;
  int ascent = 8;
  int descent = 0; // Skia path keeps this positive
  ...
  const int text_h = ascent + descent;
  const int shaped_text_len = sf ? swell_skia_simple_text_run_len(buf, buflen, align) : -1;
  ...
  if (align&DT_CALCRECT)
  {
    ...
    if (shaped_text_len > 0)
    {
      float adv = 0.0f, ink_r = 0.0f;
      if (SWELL_SkiaMeasureTextRun(sf, buf, shaped_text_len, &adv, NULL, &ink_r))
      {
        const int ext = wdl_max(swell_skia_round_text_extent(adv, charw),
                                (int)(ink_r + 0.5f));
        r->right = wdl_max(r->right, r->left + ext);
        r->bottom = wdl_max(r->bottom, r->top + text_h);
  ...
  if (shaped_text_len > 0)
  {
    float adv = 0.0f, ink_l = 0.0f, ink_r = 0.0f;
    if (SWELL_SkiaMeasureTextRun(sf, buf, shaped_text_len, &adv, &ink_l, &ink_r))
    {
      const int ha = swell_skia_round_text_extent(adv, charw);
      if (bgmode==OPAQUE)
        SWELL_SkiaFillRect(surface,xpos,ypos,ha,
                           (align&DT_SINGLELINE) ? text_h : lineh,
                           bgcol,1.0f);

      SWELL_SkiaDrawTextRun(surface, sf, buf, shaped_text_len,
                            (float)xpos, (float)(ypos + ascent), fgcol);

      const int dirty_l = wdl_min(xpos, xpos + (int)ink_l);
      int max_xpos = wdl_max(xpos + ha, xpos + (int)(ink_r + 0.5f));
      if (max_xpos <= dirty_l) max_xpos = dirty_l + ha;
      const int max_ypos = ypos + text_h;

      if (surface==&clipbm)
        swell_DirtyContext(ct,clip_x1+dirty_l,clip_y1+ypos,clip_x1+max_xpos,clip_y1+max_ypos);
      else
        swell_DirtyContext(ct,dirty_l,ypos,max_xpos,max_ypos);

      return max_ypos - ypos;
    }
  }
  ...
  while (buflen && *buf)
  {
    ...
    {
      if (bgmode==OPAQUE)
        SWELL_SkiaFillRect(surface,xpos,ypos,charw*5,
                           (align & DT_SINGLELINE) ? text_h : lineh,
                           bgcol,1.0f);

      xpos+=charw*5;

      const int bext = ypos+text_h;
    ...
    else
    {
      ...
      {
        // Strict Skia path: no FreeType/LICE glyph fallback here.
        // Advance/layout is still preserved for dirty rect behavior.
        if (bgmode==OPAQUE)
          SWELL_SkiaFillRect(surface,xpos,ypos,charw,
                             (align & DT_SINGLELINE) ? text_h : lineh,
                             bgcol,1.0f);
        ...
        const int bext=ypos+text_h+(doUl ? 2:1);
        if (max_ypos < bext) max_ypos=bext;
        xpos+=charw;
      }
    }
